...
private:
  ...
  bool isVoidPointerConversion(clang::QualType From, clang::QualType To) const {
    if (!From->isPointerType() || !To->isPointerType()) return false;
  ...
  bool isUnrelatedPointers(clang::QualType From, clang::QualType To) const {
    ...
    if (FromPointee.getUnqualifiedType() == ToPointee.getUnqualifiedType())
      return false;
    if (areRelatedClasses(FromPointee, ToPointee)) return false;
    if (FromPointee->isVoidType() || ToPointee->isVoidType()) return false;
    return true;
  }

  bool isConstVolatileAdjustment(clang::QualType From, clang::QualType To) const {
  ...
  bool areRelatedClasses(clang::QualType A, clang::QualType B) const {
    ...
    if (!DeclA || !DeclB) return false;
    return DeclA == DeclB || DeclA->isDerivedFrom(DeclB) || DeclB->isDerivedFrom(DeclA);
  }

  bool isDowncastForPolymorphic(clang::QualType From, clang::QualType To) const {
    ...
    if (!FromDecl || !ToDecl) return false;
    FromDecl = FromDecl->getDefinition();
    ToDecl   = ToDecl->getDefinition();
    if (!FromDecl || !ToDecl) return false;
    if (!FromDecl->isPolymorphic()) return false;
...
public:
  ...
  std::string build(const clang::CStyleCastExpr *cast, CastKind kind) const {
    ...
    if (typeText.empty() || exprText.empty()) return {};
...
private:
  ...
  std::string getTypeTextFromCast(const clang::CStyleCastExpr *cast) const {
    clang::SourceRange typeRange = cast->getTypeInfoAsWritten()->getTypeLoc().getSourceRange();
  ...
  std::string getSourceText(clang::SourceRange range) const {
    ...
    return clang::Lexer::getSourceText(clang::CharSourceRange::getTokenRange(range), SM, LO).str();
  }
};

class CastRewriterVisitor : public clang::RecursiveASTVisitor<CastRewriterVisitor> {
public:
...
static clang::FrontendPluginRegistry::Add<CastRewriterAction>
    X("cast_rewriter_mityaeva", "Rewrite C-style casts to C++ casts (Mityaeva Darya)");