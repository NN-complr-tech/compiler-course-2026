...
private:
  ...
  bool evaluate(clang::FunctionDecl* func) {
    ...
    if (record.isProcessed) return record.canThrow;
    if (!visitStack.insert(func).second) return false;

    if (auto* proto = func->getType()->getAs<clang::FunctionProtoType>()) {
      if (proto->getExceptionSpecType() != clang::EST_None) {
        record.canThrow = (proto->getExceptionSpecType() != clang::EST_BasicNoexcept);
...
public:
  bool mayThrow(clang::FunctionDecl* func) {
...
public:
  ...
  bool VisitFunctionDecl(clang::FunctionDecl* function) {
    if (!isEligible(function)) return true;
...
private:
  bool isEligible(clang::FunctionDecl* function) {
    ...
    if (function->isImplicit() || function->isDeleted() || function->isDefaulted()) return false;
    if (m_sourceManager.isInSystemHeader(function->getLocation())) return false;
  ...
  void applyNoexcept(clang::FunctionDecl* function) {
    ...
    clang::FunctionProtoType::ExtProtoInfo extensionInfo = proto->getExtProtoInfo();
...
public:
  ...
  CreateASTConsumer(clang::CompilerInstance& instance, llvm::StringRef) override {
