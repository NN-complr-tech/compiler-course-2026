...
public:
    ...
    bool canThrow(const clang::Stmt* stmt, const FunctionSet& noexceptFns) {
        ...
        while (!worklist.empty()) {
            ...
            if (const auto* construct = llvm::dyn_cast<clang::CXXConstructExpr>(current)) {
...
private:
    bool isCalleeSafe(const clang::FunctionDecl* fn, const FunctionSet& noexceptFns) const {
        if (!fn) return true;
...
static clang::FrontendPluginRegistry::Add<NoexceptPluginAction>
    X("noexcept_analyzer", "Add noexcept specifier to functions that don't throw");