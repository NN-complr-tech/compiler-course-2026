...
public:
  explicit ExampleVisitor(ASTContext *context) 
      : m_context(context), m_rewriter(context->getSourceManager(), context->getLangOpts()) {}
  ...
  void outputResult() {
    ...
    if (const auto *buffer = m_rewriter.getRewriteBufferFor(mainFile)) {
      ...
      bool invalid = false;
      StringRef original = m_context->getSourceManager().getBufferData(mainFile, &invalid);
      if (!invalid) {
        llvm::outs() << original;
      }
    }
...
static FrontendPluginRegistry::Add<ExampleAction>
    X("nikitin_a_lab_1_plugin", "Add const to unmodified pointers and references");