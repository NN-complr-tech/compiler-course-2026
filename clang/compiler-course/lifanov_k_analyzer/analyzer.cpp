...
public:
    ...
    bool VisitReturnStmt(ReturnStmt *ret) {
        for (auto *var : allocated) {
            if (released.find(var) == released.end() && reported.find(var) == reported.end()) {
                warn(ret->getReturnLoc(), "ресурс для переменной '%0' может быть не освобождён при выходе");
                reported.insert(var);
            }
        }
        return true;
    ...
    void finalize() {
        for (auto *var : allocated) {
            if (released.find(var) == released.end() && reported.find(var) == reported.end()) {
                warn(var->getLocation(), "выделенный ресурс для '%0' не освобождён");
            }
        }
    }
...
private:
    ...
    VarDecl *extractVar(Expr *expr) {
        ...
        if (auto *ref = dyn_cast<DeclRefExpr>(expr)) return dyn_cast<VarDecl>(ref->getDecl());
