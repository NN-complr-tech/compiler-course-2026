...
class OvsyannikovOverrideVisitor : public RecursiveASTVisitor<OvsyannikovOverrideVisitor> {
public:
    ...
    bool VisitCXXMethodDecl(CXXMethodDecl *Method) {
        if (Method->size_overridden_methods() > 0 && 
            ...
            unsigned DiagID = DiagEngine.getCustomDiagID(
                ...
                "ovsyannikov-check: method '%0' overrides a virtual function but lacks 'override' specifier"
            );

            DiagEngine.Report(Method->getLocation(), DiagID) << Method->getNameAsString();
        }
        return true;
    }
...
public:
    explicit OvsyannikovOverrideConsumer(ASTContext *Context) : Visitor(Context) {}
