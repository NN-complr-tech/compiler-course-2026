...
int computeMaxDepth(Region &region, int currentDepth) {
  ...
  for (Operation &op : region.getOps()) {
    ...
    if (isa<scf::IfOp, scf::ForOp, scf::WhileOp,
...
class FedoseevPass4 : public PassWrapper<FedoseevPass4, OperationPass<ModuleOp>> {
public:
  StringRef getArgument() const final { return "FedoseevPass4"; }
  StringRef getDescription() const final {
    return "Attach max depth of scf/affine if/for/while blocks to each function";
  }
