...
struct OvsyannikovLoopFusion : public OpRewritePattern<scf::ForOp> {
  ...
  LogicalResult matchAndRewrite(scf::ForOp firstLoop, PatternRewriter &rewriter) const override {
    ...
    for (Value res : firstLoop.getResults()) {
      for (Operation *user : res.getUsers()) {
        if (secondLoop->isAncestor(user)) return failure();
      }
    }
...
class OvsyannikovSCFPass : public PassWrapper<OvsyannikovSCFPass, OperationPass<ModuleOp>> {
public:
  ...
  StringRef getDescription() const final { return "Fuses adjacent scf.for loops"; }
...
extern "C" LLVM_ATTRIBUTE_WEAK mlir::PassPluginLibraryInfo mlirGetPassPluginInfo() {
  return {MLIR_PLUGIN_API_VERSION, "OvsyannikovSCF", "1.0", []() { mlir::PassRegistration<OvsyannikovSCFPass>(); }};
}
