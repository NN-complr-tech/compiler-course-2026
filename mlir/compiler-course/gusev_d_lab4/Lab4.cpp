...
public:
  ...
  void runOnOperation() override {
    ...
    Module.walk([&](Operation *Op) {
      ...
      if (Name != "scf.if" && Name != "affine.if") {
        return;
      }

      instrumentRegion(Op->getRegion(0), "trace_condition_then_begin",
                       "trace_condition_then_end");
      if (Op->getNumRegions() > 1 && !Op->getRegion(1).empty()) {
        instrumentRegion(Op->getRegion(1), "trace_condition_else_begin",
...
private:
  ...
  static void instrumentRegion(Region &Region, StringRef BeginName,
    ...
    Builder.create<func::CallOp>(Block.front().getLoc(), BeginName,
