...
func.func @just_function(%x: i64, %y: i64) -> i64 {
  %result = arith.muli %x, %y : i64
  return %result : i64
}
...
func.func @if_with_else(%predicate: i1) -> i32 {
  %value = scf.if %predicate -> i32 {
    %const5 = arith.constant 5 : i32
    scf.yield %const5 : i32
  } else {
    %const3 = arith.constant 3 : i32
    scf.yield %const3 : i32
  }
  return %value : i32
}
...
func.func @single_while(%initial: i32, %max_val: i32) -> i32 {
  %result = scf.while (%current = %initial) : (i32) -> i32 {
    %is_less = arith.cmpi ult, %current, %max_val : i32
    scf.condition(%is_less) %current : i32
  } do {
  ^bb0(%current: i32):
    %two = arith.constant 2 : i32
    %next_val = arith.subi %current, %two : i32
    scf.yield %next_val : i32
  }
  return %result : i32
}
...
func.func @for_with_while_nested(%lb: index, %ub: index, %step: index) {
  scf.for %i = %lb to %ub step %step {
    %ignored = scf.while (%cur = %i) : (index) -> index {
      %cond = arith.cmpi slt, %cur, %ub : index
...
func.func @while_with_if_nested(%init: index, %limit: index, %flag: i1) {
  %ignored = scf.while (%cur = %init) : (index) -> index {
    %cond = arith.cmpi slt, %cur, %limit : index
    scf.condition(%cond) %cur : index
  } do {
    ^bb0(%cur: index):
    scf.if %flag {
...
func.func @if_with_while_nested(%cond: i1, %start: index, %limit: index) {
  scf.if %cond {
    %ignored = scf.while (%cur = %start) : (index) -> index {
      %cmp = arith.cmpi ne, %cur, %limit : index
      scf.condition(%cmp) %cur : index
    } do {
      ^bb0(%cur: index):
      %next = arith.subi %cur, %start : index
      scf.yield %next : index
    }
  }
  return
...
func.func @if_with_while_in_else(%flag: i1, %init: index, %bound: index) {
  scf.if %flag {
    ...
    %ignored = scf.while (%cur = %init) : (index) -> index {
      %cond = arith.cmpi sgt, %cur, %bound : index
      scf.condition(%cond) %cur : index
    } do {
      ^bb0(%cur: index):
      %dec = arith.constant 1 : index
      %next = arith.subi %cur, %dec : index
      scf.yield %next : index
    }
  }
  return
...
func.func @complex_nesting_4(%lb: index, %ub: index, %step: index, %flag: i1) {
  scf.if %flag {
    scf.for %i = %lb to %ub step %step {
      %ignored = scf.while (%cur = %i) : (index) -> index {
        %cond = arith.cmpi ne, %cur, %ub : index
        scf.condition(%cond) %cur : index
      } do {
        ^bb0(%cur: index):
    ...
    %ignored = scf.while (%w = %lb) : (index) -> index {
      %cnd = arith.cmpi ult, %w, %ub : index
      scf.condition(%cnd) %w : index
    } do {
      ^bb0(%w: index):
...
func.func @nested_ifs_and_for(%cond1: i1, %cond2: i1, %lb: index, %ub: index, %step: index) {
  scf.if %cond1 {
    scf.if %cond2 {
      scf.for %i = %lb to %ub step %step {
...
// CHECK-SAME: max_block_depth = 2 : i64
func.func @for_with_if(%lb: index, %ub: index, %step: index, %cond: i1) {
  scf.for %i = %lb to %ub step %step {
    scf.if %cond {
...
// CHECK-SAME: max_block_depth = 2 : i64
func.func @while_with_for(%init: index, %limit: index, %lb: index, %ub: index, %step: index) {
  %ignored = scf.while (%cur = %init) : (index) -> index {
    %cond = arith.cmpi slt, %cur, %limit : index
    scf.condition(%cond) %cur : index
  } do {
...
// CHECK-SAME: max_block_depth = 1 : i64
func.func @code_around_loop(%lower_bound: index, %upper_bound: index, %step_count: index) -> i32 {
